...
| **Path**                    | **Description**                                                                                                                                                                                                                                                                                                                              |
| :-------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/var/www/html`             | Similar to NGINX, this is where all your publicly available files should be, including any asset. This path was chosen since it is familiar to most people, you can change this by overriding the default config and setting `root` to anywhere you want in the container                                                                |
...
| `root`              | `string`  | This is where your static files and assets are located (with additional options), for example `/var/www/html`. This is relative to where the config is located if it is not an absolute path, for example, if your config is located at `/Users/name/personal/chimney.toml` and the `root` is set to `"public"`, this will be resolved to `/Users/name/personal/public`                                                                                                                     |   `"public"`   |
