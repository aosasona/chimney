...
impl Server {
    ...
    pub fn get_file_path(&self, target: &str) -> Option<PathBuf> {
...
async fn serve_file(
    ...
    let request_path = if req.uri().path().trim_end_matches("/").is_empty() {
        "/index.html"
    } else {
        req.uri().path()
    };
    ...
    let path = match server.get_file_path(target.as_str()) {
        Some(path) => path,
        None => {
            return Ok(not_found());
        }
    ...
    // TODO: refactor into own function (file_to_response)
    //
    // TODO: fetch proper mimetype
